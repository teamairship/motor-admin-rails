<template>
  <div class="bg-white">
    <Tabs v-model="selectedTab" :position="'center'" :tabs="[
    ...
    <div :style="{
      ...
      <template v-if="selectedTab === 'variables'">
        <template v-if="withVariablesEditor || preferences.variables.length">
          <Card v-for="(variable, index) in preferences.variables" :key="index" class="my-2">
            <div v-if="!withVariablesEditor" class="fw-bold mb-3">
              {{ variable.display_name }}
            </div>
            <VariableSettings :variable="variable" :with-name="withVariablesEditor" :with-remove="withVariablesEditor"
              @remove="preferences.variables.splice(index, 1)" />
          </Card>
          <VButton v-if="withVariablesEditor" icon="md-add" long @click="addNewVariable">
            {{ i18n.add_variable }}
          </VButton>
        </template>
      ...
      <template v-else>
        ...
        <RadioGroup v-model="preferences.visualization" class="d-flex flex-column">
          <Radio v-for="option in visualizationOptions" :key="option.value" :label="option.value" border size="large"
            class="my-1 me-0" @update:model-value="maybeSetLabelFormat">
            {{ option.label }}
          </Radio>
        </RadioGroup>
        <template v-if="['bar_chart', 'row_chart', 'line_chart', 'funnel'].includes(preferences.visualization)">
          <Checkbox v-model="preferences.visualization_options.chart_values_axis" class="d-block">
            {{ ' ' }} {{ i18n['values_axis'] }}
          </Checkbox>
          <Checkbox v-if="['bar_chart', 'row_chart'].includes(preferences.visualization)"
            v-model="preferences.visualization_options.chart_stacked" class="d-block">
            {{ ' ' }} {{ i18n['stacked_bars'] }}
          </Checkbox>
        </template>
        <template v-if="!['table', 'markdown', 'html', 'value', 'map', 'iframe'].includes(preferences.visualization)">
          ...
          <RadioGroup v-model="preferences.visualization_options.label_format" class="d-flex flex-column"
            @update:model-value="maybeSetCurrency">
            <Radio v-for="option in formatOptions" :key="option.value" :label="option.value" border size="large"
              class="my-1 me-0">
              {{ option.label }}
            </Radio>
          </RadioGroup>
        ...
        <template v-if="!['table', 'markdown', 'html', 'iframe'].includes(preferences.visualization) && preferences.visualization_options.label_format === 'currency'">
          ...
          <CurrencySelect v-model="preferences.visualization_options.label_format_options.currency" />
        </template>
      </template>
    </div>
    <div class="footer p-2">
      <VButton long type="default" @click="$emit('close')">
        {{ i18n['ok'] }}
      </VButton>
    </div>
  </div>
